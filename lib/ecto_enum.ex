defmodule EctoEnum do
  ...
  def cast(val, int_atom_map, string_atom_map) do
    if val in Map.keys(int_atom_map) do
      Map.fetch(int_atom_map, val)
    else
  ...
  def dump(val, _, string_int_map, int_atom_map) do
    if val in Map.keys(int_atom_map) do
      {:ok, val}
    else
      Map.fetch(string_int_map, val)
    end
  end
end
